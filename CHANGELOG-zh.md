...
### 发布概览
- 恢复 Island UI 原定的首次启动浅色外观。

### 用户可见更新
- **新配置现在默认以浅色主题启动。** 未保存主题偏好时，Patchbay 使用浅灰画布与白色内容岛；用户主动选择的浅色、深色或跟随系统仍保持不变。

### 开发者与治理更新
- 增加无主题偏好时的默认值回归测试，防止后续视觉改版再次把旧的深色 fallback 带回。

## [2.0.1] - 2026-08-11

### 发布概览
- 在 2.0 收敛后的产品范围内强化链路修复安全，并刷新 PostCSS 工具链，不扩张 Patchbay 的功能边界。

### 用户可见更新
- **不可用的 Agent 面不再被改写并误报为已修复。** 当面链接目标正确，但聚合目录缺失、不可访问或暂时不可读时，现在会报告冲突并保持原链接不动。

### 开发者与治理更新
- 链接重建的最终竞态检查失败且公开路径仍为空时，会恢复另一写入者并发替换的链接，不再把它滞留在隐藏备份名下。
- `RepairDirLink` 仅用于它确实能够修正的 Windows 非原生链接形态；macOS 与 Windows 回归测试覆盖两项修复安全语义。
- PostCSS 从 8.5.22 更新至 8.5.26，包含 source map 路径保护修复及相应的 Nano ID 补丁更新。
