...
/// the commit trailers plus `refs/patchbay/conflict/*`, from which
...
/// v7 → v8: repair journal (issue #31). One row per chain-repair apply that
/// actually wrote, carrying the applied items verbatim — the inverse operation
/// is fully derivable from each item's `path`/`old_target`/`new_target`, so
/// the row IS the undo record. `status` flips to `undone` after a rollback;
/// `dismissed` hides the record card without deleting the history.
...
/// v8 → v9: chain assembly presets (issue #35). A named set of warehouse
/// skill references — `skills` is a JSON array of {name, path, repo} kept
/// opaque here, parsed by the chain preset module. Names are unique; the
/// wizard (#36) consumes these as batch-link starting points.
