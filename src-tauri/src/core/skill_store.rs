...
/// One raw repair-journal row (issue #31). `projects`/`fingerprints`/`items`
/// are JSON strings the chain journal module parses — kept opaque here.
#[derive(Debug, Clone)]
pub struct RepairJournalRow {
    pub id: i64,
    pub created_at: i64,
    pub projects: String,
...
fn map_repair_journal_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<RepairJournalRow> {
    Ok(RepairJournalRow {
        id: row.get(0)?,
        created_at: row.get(1)?,
        projects: row.get(2)?,
        fingerprints: row.get(3)?,
        items: row.get(4)?,
        verified: row.get::<_, i32>(5)? != 0,
        status: row.get(6)?,
        dismissed: row.get::<_, i32>(7)? != 0,
    })
}

/// One raw chain-preset row (issue #35); `skills` is JSON the chain preset
/// module parses.
#[derive(Debug, Clone)]
pub struct ChainPresetRow {
    pub id: i64,
    pub name: String,
    pub skills: String,
    pub created_at: i64,
...
impl SkillStore {
    ...
    // ── Repair journal (issue #31) ──
    // One row per chain-repair apply that wrote something. Rows carry the
    // applied items as opaque JSON — (de)serialization lives with the chain
    // journal module so this store stays type-agnostic about chain shapes.

    pub fn insert_repair_journal(
        &self,
        projects_json: &str,
        ...
        let now = chrono::Utc::now().timestamp();
        conn.execute(
            "INSERT INTO repair_journal (created_at, projects, fingerprints, items, verified)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![
                now,
                projects_json,
    ...
    pub fn list_repair_journal(&self, limit: Option<i64>) -> Result<Vec<RepairJournalRow>> {
        let conn = self.conn.lock().unwrap();
        let sql =
            "SELECT id, created_at, projects, fingerprints, items, verified, status, dismissed
                   FROM repair_journal ORDER BY id DESC LIMIT ?1";
        let mut stmt = conn.prepare(sql)?;
        let rows = stmt
            .query_map(params![limit.unwrap_or(-1)], map_repair_journal_row)?
            .collect::<std::result::Result<Vec<_>, _>>()?;
        Ok(rows)
    }

    pub fn get_repair_journal(&self, id: i64) -> Result<Option<RepairJournalRow>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT id, created_at, projects, fingerprints, items, verified, status, dismissed
             FROM repair_journal WHERE id = ?1",
        )?;
        let mut rows = stmt.query_map(params![id], map_repair_journal_row)?;
        Ok(rows.next().transpose()?)
    }

    /// Flip a record's lifecycle status ("applied" → "undone").
    pub fn set_repair_journal_status(&self, id: i64, status: &str) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "UPDATE repair_journal SET status = ?2 WHERE id = ?1",
            params![id, status],
        )?;
        Ok(())
    }

    /// Hide the record's card without deleting the history.
    pub fn set_repair_journal_dismissed(&self, id: i64) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "UPDATE repair_journal SET dismissed = 1 WHERE id = ?1",
            params![id],
        )?;
        Ok(())
    }
    ...
    // Named sets of warehouse skill references; `skills` is opaque JSON the
    // chain preset module parses.

    pub fn insert_chain_preset(&self, name: &str, skills_json: &str) -> Result<i64> {
        let conn = self.conn.lock().unwrap();
        let now = chrono::Utc::now().timestamp();
        conn.execute(
            "INSERT INTO chain_presets (name, skills, created_at) VALUES (?1, ?2, ?3)",
            params![name, skills_json, now],
        )?;
        Ok(conn.last_insert_rowid())
    }

    pub fn list_chain_presets(&self) -> Result<Vec<ChainPresetRow>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt =
            conn.prepare("SELECT id, name, skills, created_at FROM chain_presets ORDER BY name")?;
        let rows = stmt
            .query_map([], |row| {
                Ok(ChainPresetRow {
                    id: row.get(0)?,
    ...
    /// Rename a preset. Returns false when no row has this id.
    pub fn rename_chain_preset(&self, id: i64, name: &str) -> Result<bool> {
        let conn = self.conn.lock().unwrap();
        let changed = conn.execute(
            "UPDATE chain_presets SET name = ?2 WHERE id = ?1",
            params![id, name],
        )?;
        Ok(changed > 0)
    }

    /// Delete a preset. Returns false when no row has this id.
    pub fn delete_chain_preset(&self, id: i64) -> Result<bool> {
        let conn = self.conn.lock().unwrap();
        let changed = conn.execute("DELETE FROM chain_presets WHERE id = ?1", params![id])?;
        Ok(changed > 0)
    }
