...
pub struct GitBackupVersion {
    /// Snapshot tag name (e.g. patchbay-v-20260318-153012-abc1234)
    pub tag: String,
...
fn remote_credential_env(skills_dir: &Path) -> Result<Vec<(String, String)>> {
    match raw_remote_url(skills_dir) {
        Some(url) => git_credentials::credential_env_for_url(&url),
        None => Ok(Vec::new()),
    }
}
...
/// Delete `refs/patchbay/*` copies that a mirror / push-all style
...
pub fn prune_hidden_refs_on_remote(skills_dir: &Path) -> Result<usize> {
    ensure_repo(skills_dir)?;
    const HIDDEN_PREFIX: &str = "refs/patchbay/";
...
pub(crate) fn push_unlocked(skills_dir: &Path) -> Result<()> {
    ...
    let local_snapshot_tags: Vec<String> = run_git(skills_dir, &["tag", "--list", "patchbay-v-*"])?
        .lines()
        .map(str::trim)
        .filter(|t| !t.is_empty())
        .map(|t| t.to_string())
    ...
    if !local_snapshot_tags.is_empty() {
        let remote_snapshot_tags_raw = run_git_env(
            skills_dir,
            &["ls-remote", "--tags", "--refs", "origin", "patchbay-v-*"],
            &env,
        )
        .unwrap_or_default();
    ...
    if !local_snapshot_tags.is_empty() {
        let remote_snapshot_tags: std::collections::HashSet<String> =
            git2_engine::ls_remote_refs(url)
                ...
                .filter(|t| t.starts_with("patchbay-v-"))
                .collect();
...
pub(crate) fn create_snapshot_tag_unlocked(skills_dir: &Path) -> Result<String> {
    ...
    let mut tag = format!("patchbay-v-{}-{}", timestamp, short_sha);
    ...
    .is_ok()
    {
        ...
        tag = format!("patchbay-v-{}-{:03}-{}", timestamp, millis, short_sha);
    }
...
pub fn list_snapshot_versions(
    ...
    let tags = run_git(
        skills_dir,
        &["tag", "--list", "patchbay-v-*", "--sort=-creatordate"],
    )?;
...
/// patchbay process attempting to populate the target between check
...
pub(crate) fn clone_into_unlocked(skills_dir: &Path, url: &str) -> Result<()> {
    ...
    let clone_result: Result<()> = if git2_engine::applies_to(url) {
        ...
        match git_credentials::credential_env_for_url(url) {
            Ok(env) => {
                let output = git_command()
                    .arg("clone")
                    .arg(url)
...
/// output, skipping dot-entries (`.patchbay` metadata, `.gitignore`).
...
const OVERSIZED_SECTION_BEGIN: &str =
    "# patchbay: oversized skills excluded from backup (auto-managed)";
const OVERSIZED_SECTION_END: &str = "# patchbay: end oversized skills";
...
pub(crate) fn apply_oversized_exclusions(skills_dir: &Path, limit: u64) -> Result<Vec<String>> {
    ...
    if !dirs.is_empty() {
        ...
        let meta_dir = skills_dir.join(".patchbay/skills");
        if meta_dir.is_dir() {
        ...
        for (rel, bytes) in dirs {
            ...
            if let Some(id) = id_by_path.get(&rel) {
                lines.push(format!("/.patchbay/skills/{}.json", gitignore_escape(id)));
            }
            log::info!(
...
fn remove_tmp_metadata_files(skills_dir: &Path) {
    let meta_root = skills_dir.join(".patchbay");
    if !meta_root.exists() {
        return;
    }
...
fn ensure_gitignore(skills_dir: &Path) -> Result<()> {
    let gitignore = skills_dir.join(".gitignore");
    let required = [".DS_Store", "Thumbs.db", "*.tmp", ".patchbay.lock"];
...
mod tests {
    ...
    fn device_email_slugs_name_with_fallback() {
        assert_eq!(device_email("MacBook Pro"), "macbook-pro@patchbay.local");
        assert_eq!(device_email("公司 Windows"), "windows@patchbay.local");
        assert_eq!(device_email("公司"), "device@patchbay.local");
    }
    ...
    fn oversized_untracked_skill_is_excluded_but_tracked_one_is_not() {
        ...
        let meta_dir = dir.join(".patchbay/skills");
        ...
        assert!(
            gitignore.contains("/.patchbay/skills/skill-huge.json"),
            "{gitignore}"
        );
        assert!(
        ...
        assert!(run_git(
            dir,
            &["cat-file", "-e", "HEAD:.patchbay/skills/skill-huge.json"]
        )
        .is_err());
    ...
    fn app_commits_carry_protocol_marker_and_trailer() {
        ...
        run_git(dir, &["cat-file", "-e", "HEAD:.patchbay/protocol.json"]).unwrap();
        ...
        assert!(safety.starts_with("patchbay-v-"));
        let body = run_git(dir, &["log", "-1", "--format=%B"]).unwrap();
    ...
    fn parse_restored_tag_valid() {
        let msg = "restore: switch skills library to patchbay-v-20260318-153012-abc1234";
        assert_eq!(
            parse_restored_from_tag_message(msg).as_deref(),
            Some("patchbay-v-20260318-153012-abc1234")
        );
    }
    ...
    fn count_changed_top_dirs_skips_metadata_and_root_dotfiles() {
        let porcelain = " M .patchbay/skills/x.json\n M .gitignore\n";
    ...
    fn restore_creates_safety_point_capturing_dirty_tree() {
        ...
        assert!(safety_tag.starts_with("patchbay-v-"));
    ...
    fn prune_hidden_refs_removes_remote_copies_and_keeps_local_refs() {
        ...
        run_git_checked(
            &work,
            &["update-ref", "refs/patchbay/conflict/skill-x", "HEAD"],
        )
        .unwrap();
        run_git_checked(&work, &["update-ref", "refs/patchbay/pre-merge", "HEAD"]).unwrap();
        run_git_checked(
            &work,
            &[
                "push",
                "origin",
                "refs/patchbay/conflict/skill-x",
                "refs/patchbay/pre-merge",
            ],
        )
        .unwrap();
        let leaked = run_git(&work, &["ls-remote", "origin", "refs/patchbay/*"]).unwrap();
        assert_eq!(
            leaked.lines().count(),
            2,
        ...
        assert_eq!(removed, 2);
        let after = run_git(&work, &["ls-remote", "origin", "refs/patchbay/*"]).unwrap();
        assert!(
            after.trim().is_empty(),
            "remote hidden refs must be gone: {after}"
        );
        // Branch and local functional refs are untouched.
        run_git(&work, &["rev-parse", "refs/patchbay/conflict/skill-x"]).unwrap();
        run_git(&work, &["rev-parse", "refs/patchbay/pre-merge"]).unwrap();
