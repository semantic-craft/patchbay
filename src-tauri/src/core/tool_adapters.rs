...
pub fn default_tool_adapters() -> Vec<ToolAdapter> {
    vec![
        ...
        ToolAdapter {
            ...
            // skills synced there by other adapters (or by older patchbay
        ...
        ToolAdapter {
            // QoderWork is a separate desktop assistant from Qoder CLI. Its
            // documented user Skill surface is ~/.qoderwork/skills/. Live app
            // verification also confirms the workspace surface `.qoder/skills`.
            key: "qoderwork".into(),
            display_name: "QoderWork CN".into(),
            relative_skills_dir: ".qoderwork/skills".into(),
            relative_detect_dir: ".qoderwork".into(),
            additional_scan_dirs: vec![],
            override_skills_dir: None,
            category: ToolCategory::Lobster,
            is_custom: false,
            recursive_scan: false,
            project_relative_skills_dir: Some(".qoder/skills".into()),
        },
    ]
}
...
mod tests {
    ...
    #[test]
    fn qoderwork_uses_distinct_project_and_global_skill_paths() {
        let adapter = default_tool_adapters()
            .into_iter()
            .find(|adapter| adapter.key == "qoderwork")
            .expect("qoderwork adapter should exist");

        assert_eq!(adapter.display_name, "QoderWork CN");
        assert_eq!(adapter.relative_skills_dir, ".qoderwork/skills");
        assert_eq!(adapter.relative_detect_dir, ".qoderwork");
        assert_eq!(adapter.category, ToolCategory::Lobster);
        assert!(!adapter.is_custom);
        assert!(!adapter.recursive_scan);
        assert_eq!(adapter.project_relative_skills_dir(), ".qoder/skills");
    }
