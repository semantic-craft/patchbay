...
pub fn hash_entries(entries: &[ContentEntry]) -> String {
    ...
    for entry in entries {
        ...
        // Folded on every platform, defaulting to "not executable" where the
        // filesystem has no such concept: gating this on `cfg(unix)` made
        // Windows skip four bytes that macOS always contributed, so the same
        // skill hashed differently per OS and every mixed-fleet comparison
        // (`sync_engine::is_target_current`, fleet sync) saw a phantom change.
        // `unwrap_or(0)` reproduces the macOS bytes for a non-executable file
        // exactly, so existing hashes there are unchanged.
...
mod tests {
    ...
    /// The mixed-fleet invariant: Windows reports `None` for every file while
    /// macOS reports `Some(0)` for a non-executable one. Both must hash the
    /// same, or the identical skill reads as "changed" on the other machine.
    /// A real executable bit must still move the hash.
    #[test]
    fn absent_and_zero_exec_bits_hash_identically() {
        let tmp = tempdir().unwrap();
        let file = tmp.path().join("a.txt");
        fs::write(&file, "hello").unwrap();
        let entry = |exec_bits| ContentEntry {
        ...
        assert_eq!(
            hash_entries(&[entry(None)]),
            hash_entries(&[entry(Some(0))]),
            "no exec bit and a zero exec bit must be the same content"
        );
        assert_ne!(
            hash_entries(&[entry(Some(0))]),
            hash_entries(&[entry(Some(0o111))]),
            "a permission-only change must still be detected"
        );
    }
}
