...
impl Device {
    ...
    fn write_skill_full(&self, id: &str, path: &str, content: &str, enabled: bool, tags: &[&str]) {
        ...
        let meta_dir = self.skills.join(".patchbay/skills");
        ...
        .unwrap();
        std::fs::write(
            self.skills.join(".patchbay/schema.json"),
    ...
    fn remove_skill(&self, id: &str, path: &str) {
        ...
        let meta = self.skills.join(format!(".patchbay/skills/{id}.json"));
        if meta.exists() {
    ...
    fn meta_of(&self, id: &str) -> Option<SkillMetaFile> {
        let raw = std::fs::read_to_string(self.skills.join(format!(".patchbay/skills/{id}.json")))
            .ok()?;
...
fn true_conflict_keeps_ours_declares_trailer_and_pins_theirs() {
    ...
    assert!(
        head_msg.contains("Patchbay-Conflicts: skill-1"),
        "{head_msg}"
    );
...
fn resolve_keep_local_closes_pending_across_devices() {
    ...
    assert_eq!(a.skill_md("alpha"), "edited on A");
    assert!(a.head_message().contains("Patchbay-Resolved: skill-1"));
...
fn resolve_keep_both_duplicates_under_device_suffix() {
    ...
    let meta_dir = a.skills.join(".patchbay/skills");
...
fn legacy_dirt_does_not_brick_the_merge() {
    ...
    std::fs::write(
        b.skills
            .join(".patchbay/skills/skill-1.json.tmp.0000-dead-beef"),
...
fn recovery_rescues_user_edits_made_after_crash() {
    ...
    let tags = git(&a.skills, &["tag", "--list", "patchbay-v-*"]);
...
fn legacy_remote_without_protocol_falls_back_to_system_merge() {
    ...
    b.activate();
    std::fs::remove_file(b.skills.join(".patchbay/protocol.json")).unwrap();
