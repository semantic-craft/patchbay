...
pub fn object_merge_pull_unlocked(store: &SkillStore, skills_dir: &Path) -> Result<MergeSummary> {
    ...
    let sig = repo
        .signature()
        .or_else(|_| git2::Signature::now("Patchbay", "patchbay@local"))?;
...
fn check_old_client_writes(
    ...
    if !doubles.is_empty() {
        bail!(
            "sync blocked: an old patchbay version performed a line-level merge on another device — commit(s) {}. Upgrade that device and retry, or use the recovery flow (a safety snapshot is taken first)",
            doubles.join("; ")
        );
    }
    ...
    for (label, tip, violated) in [
        ...
        validate::validate_input_tip(repo, &tree, &tolerated).with_context(|| {
            format!(
                "sync blocked: an old patchbay version wrote to the {label} library and left it inconsistent — upgrade that device and repair via the recovery flow"
            )
        })?;
    ...
    let listed: Vec<String> = ours_violations
        ...
        .collect();
    Ok(Some(format!(
        "old patchbay version wrote commit(s) {} — please upgrade that device",
        listed.join("; ")
    )))
}
