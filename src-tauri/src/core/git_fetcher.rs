...
pub const CLONE_TEMP_PREFIX: &str = "patchbay-clone-";
...
///
/// Shared with `core::fleet`, which shells out to git across every managed
/// repository per round — without this each call flashes a console window.
...
pub fn resolve_remote_revision(
    ...
    let repo = Repository::init_bare(
        std::env::temp_dir().join(format!("patchbay-remote-{}", uuid::Uuid::new_v4())),
    )?;
