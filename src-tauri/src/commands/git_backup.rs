...
pub(crate) fn sync_engine_pref(store: &SkillStore) {
    ...
    git_credentials::set_proxy(store.proxy_url());
}
...
pub async fn github_backup_connect(
    ...
    let store = store.inner().clone();
    sync_engine_pref(&store);
    tokio::task::spawn_blocking(move || connect_with_pat(&store, token.trim(), repo_name.trim()))
        .await?
}
...
fn connect_with_pat(
    ...
    let info = github_api::connect_pat_backup_repo(token, repo_name, proxy_url.as_deref())
        .map_err(AppError::network)?;
    finish_github_connection(
        store,
        info,
...
fn connect_with_github_app_token(
    store: &SkillStore,
    token: github_api::GithubAppToken,
    repo_name: &str,
    expected_repo_id: u64,
) -> Result<GithubBackupConnectResult, AppError> {
    if !github_api::is_valid_repo_name(repo_name) {
        return Err(AppError::invalid_input("Invalid repository name"));
    }
    let proxy_url = store.proxy_url();
    let info = github_api::connect_github_app_backup_repo(
        &token.access_token,
        repo_name,
    ...
    finish_github_connection(
        store,
        info,
        git_credentials::github_app_credential(token, expected_repo_id),
        "github_app",
    )
}

fn finish_github_connection(
    store: &SkillStore,
    info: github_api::GithubConnectInfo,
    credential: git_credentials::RemoteCredential,
    method: &str,
) -> Result<GithubBackupConnectResult, AppError> {
    git_credentials::store_credential("github.com", &credential)
...
pub struct GithubDevicePollResult {
    /// "pending" | "slow_down" | "repository_identified" | "connected"
    pub status: String,
    pub result: Option<GithubBackupConnectResult>,
    pub next_flow: Option<github_api::DeviceFlowStart>,
    pub repository_id: Option<u64>,
}

/// One device-flow poll. On authorization the GitHub App user token stays in the
/// backend: it goes straight into the OS keychain and never reaches the UI.
#[tauri::command]
pub async fn github_device_flow_poll(
    ...
    tokio::task::spawn_blocking(move || {
        let proxy_url = store.proxy_url();
        match github_api::device_flow_poll(&device_code, repository_id, proxy_url.as_deref())
            .map_err(AppError::network)?
        {
            ...
            github_api::DevicePollOutcome::Authorized(token) => {
                if let Some(expected_repo_id) = repository_id {
                    let result = connect_with_github_app_token(
                        &store,
                        token,
                        repo_name.trim(),
                        expected_repo_id,
                    ...
                    .map_err(AppError::network)?;
                    let next_flow = github_api::device_flow_start(proxy_url.as_deref())
                        .map_err(AppError::network)?;
                    Ok(GithubDevicePollResult {
                        status: "repository_identified".to_string(),
                        result: None,
                        next_flow: Some(next_flow),
                        repository_id: Some(info.repo_id),
                    })
                }
            }
        }
