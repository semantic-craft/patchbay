...
pub fn delete_managed_skills_by_ids(
    ...
    sync_metadata::with_repo_lock("delete skills", || {
        ...
        for skill_id in skill_ids {
            ...
            let chain = crate::core::chain::ChainService::new(store);
            let dependents = chain
                .projects_referencing_source(&central)
                .map_err(|error| anyhow::anyhow!(error.message))?;
            let chain_cleanup = dependents.iter().try_for_each(|project| {
                let plan = chain.plan_unlink(project, &skill.name, &[])?;
                let outcome = chain.apply_unlink(&plan)?;
                if outcome.verified {
                    Ok(())
                } else {
                    Err(AppError::internal(format!(
                        "could not verify removal of {} from {}",
                        skill.name,
                        project.display()
                    )))
                }
            });
            if let Err(error) = chain_cleanup {
                store.log_audit(
                    AuditDraft::new("remove")
...
mod tests {
    ...
    #[test]
    fn deleting_managed_skill_removes_verified_project_chain_first() {
        let repo = test_repo();
        let skill_dir = write_skill_dir("skill-one");
        repo.store
            .insert_skill(&sample_skill("skill-1", "skill-one", &skill_dir))
            .unwrap();
        let project = repo._tmp.path().join("consumer");
        fs::create_dir_all(&project).unwrap();
        let chain = crate::core::chain::ChainService::new(&repo.store);
        let linked = chain
            .link(&project, &[skill_dir.clone()], &["qoderwork".to_string()])
            .unwrap();
        assert!(linked.verified);
        assert_eq!(
            chain.projects_referencing_source(&skill_dir).unwrap(),
            [project.clone()]
        );

        let result = delete_managed_skills_by_ids(&repo.store, &["skill-1".to_string()]).unwrap();

        assert_eq!(result.deleted, 1);
        assert!(result.failed.is_empty());
        assert!(!skill_dir.exists());
        assert!(fs::symlink_metadata(project.join(".agents/skills/skill-one")).is_err());
        let qoderwork = crate::core::chain::link_tracer::trace(&project.join(".qoder/skills"));
        assert!(qoderwork.exists);
        assert_eq!(
            crate::core::chain::link_tracer::normalize(Path::new(&qoderwork.final_target)),
            crate::core::chain::link_tracer::normalize(&project.join(".agents/skills"))
        );
    }
