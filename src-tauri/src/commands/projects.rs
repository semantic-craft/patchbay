...
use crate::core::{
    error::AppError, installer, project_registry, project_scanner, sync_engine, tool_adapters,
};
...
mod tests {
    ...
    /// Portable stand-in for `std::os::unix::fs::symlink`, which these fixtures
    /// used while they were `cfg(unix)`-gated. Every one links a directory, so
    /// the junction fallback is equivalent — and it needs no Developer Mode, so
    /// the "removes the link, never the target" assertions now run on Windows,
    /// where getting that distinction wrong is exactly how a junction deletion
    /// eats the real directory.
    fn symlink(
