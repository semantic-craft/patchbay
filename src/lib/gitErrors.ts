...
export function mapGitErrorMessage(error: unknown, t: TFunction): string {
  ...
  if (message.includes("GITHUB_APP_REAUTH_REQUIRED")) {
    return t("backup.github.errorAppReauth");
  }
  if (message.includes("GITHUB_APP_REPO_NOT_PRIVATE")) {
    return t("backup.github.errorAppRepoNotPrivate");
  }
  if (message.includes("GITHUB_APP_INSTALLATION_SCOPE")) {
    return t("backup.github.errorAppInstallationScope");
  }
