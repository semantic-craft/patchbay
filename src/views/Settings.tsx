...
import { WarehouseRootsSection } from "../components/WarehouseRootsSection";
...
export function Settings() {
  ...
  const [appAutoUpdateEnabled, setAppAutoUpdateEnabled] = useState(true);
  // Agent path editing
  ...
  const GITHUB_URL = "https://github.com/semantic-craft/patchbay-releases";
  ...
  useEffect(() => {
    ...
    api.getSettings(APP_UPDATE_ENABLED_SETTING).then((v) => {
      setAppAutoUpdateEnabled(isAppAutoUpdateEnabled(v));
    });
  ...
  const handleAppAutoUpdateChange = async (enabled: boolean) => {
    setAppAutoUpdateEnabled(enabled);
    await api.setSettings(APP_UPDATE_ENABLED_SETTING, enabled ? "on" : "off");
  };
  ...
  const handleReportIssue = async () => {
    ...
    try {
      ...
      const parts = [
        "**Diagnostics** (auto-collected by Patchbay)",
        "",
  ...
  const handleAutoUpdate = async () => {
    ...
    try {
      ...
      if (result.status === "no-update") {
        ...
        toast.info(t("settings.updateAvailable", { version: result.version }));
      }
    } catch {
      toast.error(t("settings.updateError"));
  ...
  return (
    <div className="app-page app-page-narrow">
      ...
      <div className="space-y-6">
        ...
        {/* Original Repository roots */}
        <WarehouseRootsSection />
        ...
        <section>
          ...
          <div className="app-panel overflow-hidden">
            <div className="flex items-center justify-between gap-4 px-4 py-2.5">
              ...
              <div className="flex shrink-0 rounded-[4px] border border-border-subtle bg-background p-px">
                <button
                  type="button"
                  aria-pressed={appAutoUpdateEnabled}
                  onClick={() => handleAppAutoUpdateChange(true)}
                  className={cn(
                    segmentedButtonClass,
                ...
                >
                  {t("settings.appUpdate.enabled")}
                </button>
                <button
                  type="button"
                  aria-pressed={!appAutoUpdateEnabled}
                  onClick={() => handleAppAutoUpdateChange(false)}
                  className={cn(
                    segmentedButtonClass,
                    !appAutoUpdateEnabled
                      ? "bg-surface-active text-secondary"
