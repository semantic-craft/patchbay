...
const DEFAULT_GITHUB_REPO = "patchbay-backup";
const GITHUB_NEW_REPO_URL =
  "https://github.com/new?name=patchbay-backup&visibility=private";
const GITHUB_APP_INSTALL_URL = "https://github.com/apps/patchbay-backup/installations/new";
const GITHUB_TOKEN_URL =
  "https://github.com/settings/tokens/new?scopes=repo&description=Patchbay%20Backup";
type RecoveryReason = GitUpstreamHealth | "conflict";

function displaySnapshotLabel(tag: string) {
  const raw = tag.startsWith("patchbay-v-") ? tag.slice("patchbay-v-".length) : tag;
...
export function Backup() {
  ...
  const mapGithubError = (error: unknown) => {
    ...
    if (message.includes("GITHUB_APP_REPO_ACCESS")) return t("backup.github.errorAppRepoAccess");
    if (message.includes("GITHUB_APP_REPO_NOT_PRIVATE")) return t("backup.github.errorAppRepoNotPrivate");
    if (message.includes("GITHUB_APP_INSTALLATION_SCOPE")) return t("backup.github.errorAppInstallationScope");
    if (message.includes("GITHUB_APP_REAUTH_REQUIRED")) return t("backup.github.errorAppReauth");
    if (message.includes("KEYCHAIN_UNAVAILABLE")) return t("backup.github.errorKeychain");
    if (message.includes("GITHUB_APP_NOT_CONFIGURED")) return t("backup.github.errorAppNotConfigured");
  ...
  /** Wire the repo locally and either restore the existing backup or push the first one. */
  ...
  const handleDeviceFlow = async () => {
    ...
    try {
      let info = await api.githubDeviceFlowStart();
      setDeviceInfo(info);
      ...
      while (!deviceCancelRef.current && Date.now() < deadline) {
        ...
        if (poll.status === "repository_identified") {
          if (!poll.next_flow || poll.repository_id == null) {
            throw new Error(
              "GITHUB_APP_INSTALLATION_SCOPE: repository-scoped authorization could not start",
            );
          }
          repositoryId = poll.repository_id;
          info = poll.next_flow;
          intervalSec = Math.max(info.interval, 5);
          deadline = Date.now() + info.expires_in * 1000;
          setDeviceInfo(info);
          toast.info(t("backup.github.deviceRepositoryScoped"));
          void openUrl(info.verification_uri);
          continue;
        }
  ...
  // Revoke the credential on GitHub, then clear Patchbay's local remote.
  const handleRevokeAuthorization = async () => {
    setRevokeConfirmOpen(false);
    const appSettings = "https://github.com/settings/installations";
    const tokenSettings = "https://github.com/settings/tokens";
    if (authMethod === "github_app") {
      openUrl(appSettings).catch(() => {});
    } else if (authMethod === "pat") {
      openUrl(tokenSettings).catch(() => {});
    } else {
      openUrl(appSettings).catch(() => {});
      openUrl(tokenSettings).catch(() => {});
    }
  ...
  return (
    <div className="app-page">
            ...
            <section className="app-panel p-4">
              ...
              {deviceInfo ? (
                ...
                <div className="space-y-2">
                  <div className="space-y-2 rounded-[6px] border border-border-subtle bg-bg-secondary p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-[13px] font-medium text-secondary">1. {t("backup.github.stepRepoTitle")}</div>
                        <p className="mt-0.5 text-[12px] leading-4 text-muted">{t("backup.github.stepRepoDesc")}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => void openUrl(GITHUB_NEW_REPO_URL)}
                        className="inline-flex h-7 shrink-0 items-center gap-1 rounded-[4px] border border-border px-2 text-[12px] font-medium text-tertiary transition-colors hover:bg-surface-hover"
                      >
                        <ExternalLink className="h-3 w-3" />
                        {t("backup.github.stepRepoAction")}
                      </button>
                    </div>
                    <div className="flex items-start justify-between gap-3 border-t border-border-subtle pt-2">
                      <div>
                        <div className="text-[13px] font-medium text-secondary">2. {t("backup.github.stepInstallTitle")}</div>
                        <p className="mt-0.5 text-[12px] leading-4 text-muted">{t("backup.github.stepInstallDesc")}</p>
                      </div>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        {t("backup.github.stepInstallAction")}
                      </button>
                    </div>
                    <div className="border-t border-border-subtle pt-2">
                      <div className="text-[13px] font-medium text-secondary">3. {t("backup.github.stepAuthorizeTitle")}</div>
                      <p className="mt-0.5 text-[12px] leading-4 text-muted">{t("backup.github.stepAuthorizeDesc")}</p>
              ...
              <p className="mt-2 text-[12px] leading-4 text-faint">
                {t(
                  authMethod === "github_app"
                    ? "backup.disconnect.revokeHintGithubApp"
                    : authMethod === "pat"
                      ? "backup.disconnect.revokeHintPat"
                      : "backup.disconnect.revokeHintUnknown",
                )}
              </p>
            )}
            {githubRepoWebUrl && (
      ...
      <ConfirmDialog
        ...
        message={t(
          authMethod === "github_app"
            ? "backup.disconnect.revokeConfirmGithubApp"
            : authMethod === "pat"
              ? "backup.disconnect.revokeConfirmPat"
              : "backup.disconnect.revokeConfirmUnknown",
        )}
        tone="warning"
