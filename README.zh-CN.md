...
<p align="center">
  <img src="assets/demo/library.png" width="800" alt="Patchbay 技能库" />
</p>
...
- **统一技能库** — 从 Git 仓库、本地目录、`.zip` / `.skill` 文件或 [skills.sh](https://skills.sh) 市场安装技能，统一存放在 `~/.patchbay`。
- **Preset（预设）** — 将技能分组为命名 Preset。在任意工作区点击 Preset 标签，即可一键为当前 Agent 范围激活或停用其全部技能，激活的 Preset 显示 ✓，部分安装显示数量。
- **全局工作区** — 每个 Agent 都有自己的页面，列出其全局目录里的所有 Skills（包括不是通过 Patchbay 安装的），始终反映 Agent 实际看到的内容。可按 Agent 添加或移除 Skills，也可通过「全部 Agents」总览跨所有已安装 Agent 统一管理。
- **项目工作区** — 查看并管理任意项目的本地 Skills 目录，支持与中央库双向同步。支持嵌套 Skill 目录和导出时按 Agent 分配。
- **Instructions 治理** — 不新增独立工作区，直接寄生现有三处界面：**链路总览**展示逐 Agent 全局 instructions 成本，**项目链路**加入 Instructions 面板，呈现正本/入口状态并以预览弹窗执行 `normalize` / `init`，**Doctor** 则把十四条 instructions 规则与既有 chain 报告合并。CLI 通过同一 service 提供 `instructions scan / where / doctor / normalize / init` 五个命令。详见[设计正本](docs/xw-instructions-design.md)与[交付 / 发布闸台账](docs/xw-instructions-verification.md)。
...
- **备份与多设备同步** — 通过 Patchbay 自有的 GitHub 授权（或任意 Git 远端）接入私有备份仓库，之后自动备份、多台设备自动保持一致。合并以技能为单位——一台改名、另一台改内容会自动组合；真冲突不阻塞不覆盖，本机版本保留待你三选一处理。快照版本随时可恢复。
...
- **全局工作区管理每个 Agent 的全局 Skills** — 每个已安装 Agent 都有自己的全局 Skills 目录（如 Claude Code 对应 `~/.claude/skills/`）。每个 Agent 页面会列出该目录里的所有内容（包括不是通过 Patchbay 安装的 Skills），可以添加、移除或纳入管理；「全部 Agents」总览则跨 Agent 统一管理。
...
6. 如果需要历史版本或多机同步，从侧边栏打开 **备份** 页，在 GitHub 授权 Patchbay——之后备份和跨设备同步都会自动进行。
...
- **在 GitHub 授权 Patchbay**（推荐）：先创建或选定私有仓库 `patchbay-backup`，把独立品牌的 **Patchbay Backup** GitHub App 仅安装到该仓库，再授权本机。首次设置需要两次简短的 GitHub 确认：Patchbay 先识别并核验私有仓库，再由 GitHub 以该仓库的 `repository_id` 为服务端硬限制签发最终令牌；公开仓库或更宽的安装范围会被拒绝。App 只有 Contents 读写权限；短期授权凭证只存在系统钥匙串里，绝不落入文件或仓库配置。个人访问令牌仍作为备用方式保留。
...
# 三层链路——诊断并登记 Doctor 裁决
npm run cli -- --json chain doctor
npm run cli -- --json chain decide --fingerprint <fp> --action mark-private  # 只读预览
npm run cli -- --json chain decide --fingerprint <fp> --action ignore --apply

# instructions 治理（CLAUDE.md / AGENTS.md）——扫描、诊断与预览/应用修复
npm run cli -- instructions scan                       # 扫描全部已登记项目
npm run cli -- instructions scan --project /path/to/proj
npm run cli -- instructions where --project /path/to/proj          # 逐 agent 读链
npm run cli -- instructions where --project /path/to/proj --agent claude
npm run cli -- instructions doctor                                 # 全部项目 + 全局面的 findings
...
npm run cli -- instructions init --project /path/to/proj           # 预览骨架 plan
...
- `chain`：检查并修复三层链路；`decide` 以 fingerprint 登记 Doctor 裁决，`--action` 取 `mark-private|ignore`，默认只预览，只有 `--apply` 才写入
- `instructions`：instructions 面工具——`scan`（正本、逐 agent 入口状态与常驻集 token 成本、个人层清单、全局面）、`where`（逐 agent 读链，含 import 跳数）、`doctor`（十四条治理规则——未初始化 / 正本缺失或双本 / 断裂 import / 正本过长 / 硬上限风险 / 技能白名单对账 / 入口被 gitignore / 全局成本——规则 id 稳定，支持 `--severity`/`--rule` 过滤与按 fingerprint 忽略）、`normalize`（preview→apply 机械修复：正本化 / 双本机械合并 / 软链转 wrapper / 补 wrapper——写前快照、正本永不改写、内容 TOCTOU + 写目标白名单守卫）与 `init`（preview→apply 骨架搭建：AGENTS.md 骨架——绝不覆盖——各已装 agent 的 wrapper 入口，`--docs-dir` 时另建空 `docs/agents/` 目录 + 骨架指针段；只创建、幂等）

额外参数：
- `--skills-root <path>`：直接针对某个已 clone / 已导出的 skills repo 操作，而不是本机 app 默认目录。manager 的状态（DB、scenarios、cache、logs）会落在 `~/.patchbay/external/<name>-<hash>/`，按 skills root 的规范化路径分目录隔离，外部仓库本身保持干净。
...
如果 agent / 脚本直接调用 `patchbay-cli`（而不是 `npm run`），需要先把二进制放到 PATH 上：
